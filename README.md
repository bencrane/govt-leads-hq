...
- `packages/tokens` — design tokens (`@govt-leads-hq/tokens`); single source of truth
- `packages/ui` — UI primitives (`@govt-leads-hq/ui`) + Storybook
- `packages/shared` — Zod schemas (`@govt-leads-hq/shared`); the catalyst-event data seam
- `packages/eslint-plugin-govt-leads-hq` — custom ESLint rules (`no-route-geometry`)
...
bun run build       # builds @govt-leads-hq/tokens then @govt-leads-hq/ui
