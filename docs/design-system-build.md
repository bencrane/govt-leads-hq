...
This document describes the design-system foundation of the `govt-leads-hq`
...
`packages/eslint-plugin-govt-leads-hq`, the Storybook setup inside
...
|---|---|---|---|
| `packages/tokens` | `@govt-leads-hq/tokens` | Yes — `build.ts` | Design tokens → CSS variables, Tailwind preset, typed TS exports |
| `packages/ui` | `@govt-leads-hq/ui` | No — source-exported | React primitives (layout + visual) |
| `packages/shared` | `@govt-leads-hq/shared` | No — types-only | Zod schemas (the catalyst-event data contract) |
| `packages/eslint-plugin-govt-leads-hq` | `eslint-plugin-govt-leads-hq` | Yes — `tsc` | Custom ESLint rule(s) |

Dependency edges, verified from each package's `package.json`
`dependencies` / `devDependencies` / `peerDependencies`:

- `@govt-leads-hq/ui` → `@govt-leads-hq/tokens` (`workspace:*`), plus
  runtime deps `clsx` `2.1.1`, `react` `19.0.0`, `react-dom` `19.0.0`.
- `@govt-leads-hq/tokens` → no runtime dependencies; only a `typescript`
  devDependency.
- `@govt-leads-hq/shared` → `zod` `3.23.8`.
- `eslint-plugin-govt-leads-hq` → `@typescript-eslint/utils` `8.18.2`
...
- `build` — runs `@govt-leads-hq/tokens build`, then
  `@govt-leads-hq/ui build`, then both apps' builds.
...
- `storybook` / `storybook:build` — delegate to `@govt-leads-hq/ui`.
...
## 2. Design tokens — `@govt-leads-hq/tokens`
...
Verified: `bun run --filter @govt-leads-hq/tokens build` exits 0 and prints
...
@govt-leads-hq/tokens test` reports `8 pass / 0 fail`.
...
- `@govt-leads-hq/tokens` — the typed TS exports (`tokens` object + types).
- `@govt-leads-hq/tokens/css` — the generated `tokens.css`, imported by
  consumers in their own CSS.
- `@govt-leads-hq/tokens/tailwind` — the `preset.js` artifact (the
  `tokensCss` path + `tokens` re-export).

---

## 3. Primitives — `@govt-leads-hq/ui`

`@govt-leads-hq/ui` is **not** a built package. `package.json` points
...
`@govt-leads-hq/tokens` types so they cannot drift, but the mapping table
...
exported and that `cx` merges classes. `bun run --filter @govt-leads-hq/ui
test` reports `12 pass / 0 fail` (10 export checks + 2 `cx` checks).

---

## 4. `@govt-leads-hq/shared`
...
optional `location` field. `bun run --filter @govt-leads-hq/shared test`
reports `6 pass / 0 fail`.

---

## 5. The ESLint plugin — `eslint-plugin-govt-leads-hq`
...
eslint-plugin-govt-leads-hq test` reports `7 pass / 0 fail` (4 + 3 cases).
...
`govt-leads-hq` and sets `govt-leads-hq/no-route-geometry` to
...
- `preview.css` — imports `tailwindcss` and `@govt-leads-hq/tokens/css`
...
`@govt-leads-hq/tokens` and `…/css` directly.
...
Each declares `@govt-leads-hq/tokens` and `@govt-leads-hq/ui` as
`workspace:*` dependencies; `platform-app` additionally depends on
`@govt-leads-hq/shared`.
...
```css
@import "tailwindcss";
@import "@govt-leads-hq/tokens/css";
```
...
Neither app imports `@govt-leads-hq/tokens/tailwind`. Tailwind v4 reads its
theme from the `@theme` block in the imported CSS, so the `/tailwind` preset
entrypoint is not needed for app styling — the `@import
"@govt-leads-hq/tokens/css"` line is what feeds Tailwind. Each app's
...
Components are imported from the `@govt-leads-hq/ui` package entry (its
...
Wordmark.tsx` imports `{ Text }` from `@govt-leads-hq/ui`. The two other
`@govt-leads-hq/tokens` references in app source
...
`@govt-leads-hq/ui` — it styles directly with Tailwind classes that resolve
...
- **The ESLint rule's docs URL.** `RuleCreator` generates a
  ...
  exists in the `eslint-plugin-govt-leads-hq` package — the URL is a
